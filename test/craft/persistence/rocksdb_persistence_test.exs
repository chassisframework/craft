defmodule Craft.Persistence.RocksDBPersistenceTest do
  ...
  setup do
    ...
    state =
      ...
      |> RocksDBPersistence.put_metadata(%{some: :metadata})
  ...
  describe "rewind/2" do
    test "when there's nothing to be rewound", %{state: state} do
      ...
      state = RocksDBPersistence.rewind(state, 1)
      ...
      assert RocksDBPersistence.latest_index(state) == 1
      assert RocksDBPersistence.latest_term(state) == 1
    end
  ...
  test "put_metadata/2 + fetch_metadata/1", %{state: state} do
    state = RocksDBPersistence.append(state, %CommandEntry{command: 1, term: 1})
    state = RocksDBPersistence.put_metadata(state, %{current_term: 1234})

    assert {:ok, %Metadata{current_term: 1234}} = RocksDBPersistence.fetch_metadata(state)
  end
